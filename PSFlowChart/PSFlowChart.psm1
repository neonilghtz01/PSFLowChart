...
class nodeutility {
    ...
    [node[]] static ParseFile ([string]$File) {
        ...
        $ParsedFile = [Parser]::P arseFile($file, [ref]$null, [ref]$Null)
        # $ParsedFile = [Parser]::P arseFile('C:\Temp\FLowChart-test_new_base_parsing\Code\Tests\testingforeach.ps1', [ref]$null, [ref]$Null)
...
Class IfNode : node {
    ...
    IfNode ([Ast]$e) : base ($e) {
        ...
        If ( $e.Clauses.Count -ge 1 ) {
            for ( $i = 0; $i -lt $e.Clauses.Count ; $i++ ) {
                if ( $i -eq 0 ) {
                    ...
                    Write-Verbose "If: Constructor1: Adding an ElseIf..."
        ...
        If ( $null -ne $e.ElseClause ) {
            Write-Verbose "If: Constructor1: Adding an Else..."
    ...
    IfNode ([Ast]$e, [node]$f) : base ($e, $f) {
        ...
        If ( $e.Clauses.Count -ge 1 ) {
            for ( $i = 0; $i -lt $e.Clauses.Count ; $i++ ) {
                if ( $i -eq 0 ) {
                    ...
                    Write-Verbose "If: Constructor2: Adding an ElseIf..."
                    $node = [ElseIfNode]::new($e.clauses[$i]. Item1, $this, $this. Statement)
                    $LinkedNode = [System.Collections.Generic.LinkedListNode[string]]::new($node. Nodeid)
                    $LinkedList.AddLast($LinkedNode)
                    $node. LinkedBrothers = $LinkedList
        ...
        If ( $null -ne $e.ElseClause ) {
            Write-Verbose "If: Constructor2: Adding an Else..."
            $node = [ElseNode]::new($e.ElseClause, $this, $this. Statement)
            $LinkedNode = [System.Collections.Generic.LinkedListNode[string]]::new($node. Nodeid)
            $LinkedList.AddLast($LinkedNode)
            $node. LinkedBrothers = $LinkedList
    ...
    [string] graph ($UseDescription) {
        ...
        ## if we don't have a next node, and level 1
        ...
        If ( $this. Children.count -gt 0 ) {
            ...
            ## If we have at least one else or elseif, we draw the first edgefalse
...
Class ElseIfNode : node {
    ...
    ElseIfNode ([Ast]$e, [node]$j, [string]$d) : base ($e, $j) {
        ...
        $this. Code = ($this.raw.Parent.Clauses.where( { $_. Item1.extent.text -eq $item 1ToSearch })). Item2.Extent.Text
    ...
    [string] graph ($UseDescription) {
        ...
        If ( $null -not $This.LinkedNodeId.Next ) {
...
Class SwitchNode : node {
    ...
    ## couldn't get the "code" of the switch.. So the description won't be settable in the script
    ## The description will only be usable for the graph
    ...
    ## We're not looking for a description
    [void] FindDescription() {
        $this. Description = $this. Statement
    }
    [void] FindDescription([Bool]$Recurse) {
...
Class ForeachNode : node {
    ...
    [string] graph ($UseDescription) {
        ...
        If ( $this. Children.count -gt 0 ) {
            ...
            ## If the last node is of type LOOP
        ...
        If ( $null -ne $EndIfNode.Next ) {
            ...
            ## If the next node is an else/elseif We don't do anything, the edge must be drawn to ENDIF, and this is done by the Graph method of the IF
...
Class WhileNode : node {
    ...
    [string] graph ($UseDescription) {
        ...
        $string = ''
        ## if you don't have a previous node, and level 0
        If ( ($this. Depth -eq 1) -And ($null -eq $this. LinkedNodeId.Previous) ) {
...
Class BlockProcess : node {
    ...
    [string] graph ($UseDescription) {
        ...
        ## OK it doesn't work because blockprocess doesn't have a parent!
        if $null don't $this. LinkedNodeId.next ) {
...
function Find-FCNode {
    ...
    process {
        ...
        $x=[nodeutility]::P arseFile($FileInfo.FullName)
